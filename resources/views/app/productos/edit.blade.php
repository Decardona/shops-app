<x-layouts.app>
  ...
  <div class="mt-4 rounded-md p-4 shadow-md md:p-10">
    <form action="{{ route('productos.update', $producto) }}" method="POST"
      class="grid grid-cols-1 space-y-4 md:grid-cols-2 md:gap-3" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 gap-4 md:col-span-2 md:grid-cols-2 md:gap-6">
        <div class="relative rounded-md">
          ...
          <img id="preview-imagen"
            src="{{ $producto->imagen ? asset('storage/products/' . $producto->imagen) : asset('noimageproduct.png') }}"
            alt="Imagen del producto" class="aspect-video w-full rounded-md object-cover object-center" />
        </div>
        <div class="mt-4 flex flex-col gap-3 md:mt-0">
          <div class="flex justify-end md:col-span-2">
            <flux:radio.group name="activo" variant="segmented">
              <flux:radio label="Activo" icon="check" value="1"
                :checked="old('activo', $producto->activo) == 1" />
              <flux:radio label="Inactivo" icon="x-mark" value="0"
                :checked="old('activo', $producto->activo) == 0" />
            </flux:radio.group>
          </div>
      ...
      <div class="md:col-span-2">
        <flux:textarea name="descripcion" label="{{ __('Descripción') }}" placeholder="Escribe una descripción"
          rows="3" resize="none">{{ old('descripcion', $producto->descripcion) }}</flux:textarea>
      </div>
