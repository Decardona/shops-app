<div>
  <div class="mb-6">
    ...
    <div class="relative mt-1">
      <div class="rounded-md focus-within:border-yellow-500 focus-within:ring-1 focus-within:ring-yellow-500">
        <input type="text" id="producto-search" wire:model.live="search" autocomplete="off"
  ...
  @if (count($productosSeleccionados) > 0)
      ...
      <table class="w-full text-sm text-gray-700">
        <thead class="mb-2 bg-gray-100">
          <tr>
            ...
            <th class="py-2 text-start">Precio Unitario</th>
            ...
            <tr class="rounded-md py-2 even:bg-gray-50">
              ...
              <td class="rounded-md px-2">
                <button type="button" wire:click="eliminarProducto({{ $i }})"
                  class="cursor-pointer font-bold text-red-600 hover:underline">Eliminar</button>
              </td>
            </tr>
          @endforeach
        </tbody>
    ...
    @if (count($productosSeleccionados) > 0)
      <input type="hidden" name="compra_detalle" value="{{ json_encode($productosSeleccionados) }}">
    @endif
  @endif
</div>
